...
class YoloNode:
  def __init__(self, p):
    ...
    for k,v in self.model.names.items(): # a dirty hack to replace "pepper" with "orange"
      if v == "orange":
    ...
    if self.use_compressed_image:
      self.image_sub = rospy.Subscriber(self.image_topic, CompressedImage, self.callback_compressed, queue_size=1, buff_size=2**24)
    else:
...
#############################
  ...
  def process_image(self, cv_image, header):
    ...
    # TODO: ids are currently not used because there is no field for it in the published message types

    # Publish "Detections" msg
    if self.detections_pub.get_num_connections() > 0:
      dets = self.generate_detections_msg(classes, scores, boxes, masks, header)
      self.detections_pub.publish(dets)

    # Publish "Result" msg
    if self.result_pub.get_num_connections() > 0:
      result_msg = self._build_result_msg(classes, scores, boxes, masks, header)
...
#############################
  ...
  def generate_visualization(self, image, classes, scores, boxes, masks, ids=None):
      ...
      for detnum in range(len(classes)):
        if ids is None:
          color = GLASBEY[classes[detnum] % 256]
        else:
  ...
  def _build_result_msg(self, classes, scores, boxes, masks, image_header):
